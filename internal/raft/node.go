...
func (n *RaftNode) applyCommitted() {
	...
	for {
		...
		n.applyCh <- entry
san@san-Vostro-15-3510:~/Desktop/raftiq$ grep -n "func (n \*RaftNode) applyConfigurationEntryLocked" internal/raft/node.go
grep -n "func (n \*RaftNode) applyCommitted" internal/raft/node.go
1386:func (n *RaftNode) applyConfigurationEntryLocked(
1426:func (n *RaftNode) applyCommitted() {
san@san-Vostro-15-3510:~/Desktop/raftiq$ sed -n "$(grep -n "func (n \*RaftNode) applyConfigurationEntryLocked" internal/raft/node.go | cut -d: -f1),+120p" internal/raft/node.go
func (n *RaftNode) applyConfigurationEntryLocked(
        entry LogEntry,
) error {
        configuration, err := DecodeConfigurationEntry(entry.Data)
...
san@san-Vostro-15-3510:~/Desktop/raftiq$ sed -n "$(grep -n "func (n \*RaftNode) applyCommitted" internal/raft/node.go | cut -d: -f1),+100p" internal/raft/node.go
func (n *RaftNode) applyCommitted() {
        n.applyMu.Lock()
        defer n.applyMu.Unlock()
...
func (n *RaftNode) replicateTo(peerID NodeID) {
        for {
                ...
                ctx, cancel := n.rpcContext()
san@san-Vostro-15-3510:~/Desktop/raftiq$ 
		n.mu.Lock()
